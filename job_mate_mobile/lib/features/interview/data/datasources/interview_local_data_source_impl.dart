...
class InterviewLocalDataSourceImpl implements InterviewLocalDataSource {
  ...
  static String _freeformHistoryKey(String chatId) =>
      'interview_freeform_history_$chatId';
  static String _structuredHistoryKey(String chatId) =>
      'interview_structured_history_$chatId';

  @override
  Future<void> cacheUserFreeformChats(
  ...
  @override
  Future<void> cacheUserStructuredChats(
    List<InterviewSessionModel> sessions,
  ) async {
    final list = sessions.map((s) => s.toJson()).toList();
  ...
  Future<List<InterviewSessionModel>> getCachedUserStructuredChats() async {
    ...
    try {
      final list = (jsonDecode(raw) as List).cast<Map<String, dynamic>>();
      return list
          .map((m) => InterviewSessionModel.fromStructuredJson(m))
          .toList();
    } catch (_) {
  ...
  @override
  Future<void> cacheFreeformHistory(
    String chatId,
    List<InterviewMessageModel> messages,
  ) async {
  ...
  @override
  Future<List<InterviewMessageModel>> getCachedFreeformHistory(
    String chatId,
  ) async {
    ...
    try {
      final list = (jsonDecode(raw) as List).cast<Map<String, dynamic>>();
      return list.map((m) => InterviewMessageModel.fromJson(m)).toList();
    } catch (_) {
  ...
  @override
  Future<void> cacheStructuredHistory(
    String chatId,
    List<InterviewMessageModel> messages,
  ) async {
    final list = messages.map((m) => m.toJson()).toList();
  ...
  @override
  Future<List<InterviewMessageModel>> getCachedStructuredHistory(
    String chatId,
  ) async {
